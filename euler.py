...
from meshing import create_naca_4digit, create_naca_4digit_3d, create_biconvex, create_blade_row, create_circle
...
gamma = 1.4
comm = MPI.COMM_WORLD
# "forward_euler", "backward_euler", or "steady_state"
time_stepping = "steady_state"

# If time_stepping is "steady_state", then num_timesteps is ignored
num_timesteps = 200

msh, ft, boundaries = create_naca_4digit(
    comm, h_near=0.01, h_far=0.2, r_near=0.8, r_far=2.5, show_gui=False)

dim = msh.topology.dim
ds = ufl.Measure("ds", domain=msh, subdomain_data=ft)

k = 0
V = fem.functionspace(msh, ("Discontinuous Lagrange", k, (dim + 2,)))
...
V_vis = fem.functionspace(msh, ("Discontinuous Lagrange", k + 1))
V_vis_vec = fem.functionspace(msh, ("Discontinuous Lagrange", k + 1, (dim,)))
...
# Write to file
vis_data = []
for name, var, space in (("rho", rho, V_vis), ("p", p, V_vis), ("u", u, V_vis_vec), ("T", T, V_vis)):
    expr = fem.Expression(var, space.element.interpolation_points)
    f = fem.Function(space)
    file = io.VTXWriter(msh.comm, f"{name}.bp", f)
    vis_data.append((expr, f, file))

if time_stepping == "forward_euler":
    U_np1 = ufl.TrialFunction(V)
    delta_t = fem.Constant(msh, 1e-6)
    F += ufl.inner((U_np1 - U) / delta_t, v) * dx

    a = ufl.lhs(F)
    L = ufl.rhs(F)

    U_h = fem.Function(V)
    problem = LinearProblem(a, L, u=U_h, petsc_options_prefix="euler_", petsc_options={
        "ksp_type": "preonly",
        "pc_type": "lu",
    ...
    for i in range(num_timesteps):
        ...
        if i % 100 == 0:
            for (expr, f, file) in vis_data:
                f.interpolate(expr)
                file.write(t)
elif time_stepping == "steady_state":
    petsc_options = {
        "snes_type": "newtonls",
        "ksp_type": "preonly",
        "pc_type": "lu",
    ...
    problem = NonlinearProblem(
        F, U, petsc_options_prefix="euler_", petsc_options=petsc_options
    )
...
elif time_stepping == "backward_euler":
    delta_t = fem.Constant(msh, 1e-3)
    U_n = fem.Function(V)
    U_n.interpolate(U)

    F += ufl.inner((U - U_n) / delta_t, v) * dx
